...
func TestNodeIterator(t *testing.T) {
	var (
		fulldb, _  = lemodb.NewMemDatabase()
		lightdb, _ = lemodb.NewMemDatabase()
		gspec      = core.Genesis{Alloc: core.GenesisAlloc{testBankAddress: {Balance: testBankFunds}}}
		genesis    = gspec.MustCommit(fulldb)
	)
	gspec.MustCommit(lightdb)
	blockchain, _ := core.NewBlockChain(fulldb, nil, params.TestChainConfig, lemohash.NewFullFaker(), vm.Config{})
	gchain, _ := core.GenerateChain(params.TestChainConfig, genesis, lemohash.NewFaker(), fulldb, 4, testChainGen)
