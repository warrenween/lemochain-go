...
See https://github.com/LemoFoundationLtd/lemochain-go/wiki/JavaScript-Console.`,
	}
...
JavaScript API. See https://github.com/LemoFoundationLtd/lemochain-go/wiki/JavaScript-Console`,
	}
)

// localConsole starts a new glemo node, attaching a JavaScript console to it at the
// same time.
func localConsole(ctx *cli.Context) error {
	...
	if err != nil {
		utils.Fatalf("Failed to attach to the inproc glemo: %v", err)
	}
...
// remoteConsole will connect to a remote glemo instance, attaching a JavaScript
// console to it.
func remoteConsole(ctx *cli.Context) error {
	...
	if endpoint == "" {
		...
		endpoint = fmt.Sprintf("%s/glemo.ipc", path)
	}
	client, err := dialRPC(endpoint)
	if err != nil {
		utils.Fatalf("Unable to attach to remote glemo: %v", err)
	}
	config := console.Config{
		DataDir: utils.MakeDataDir(ctx),
		DocRoot: ctx.GlobalString(utils.JSpathFlag.Name),
...
// for "glemo attach" and "glemo monitor" with no argument.
func dialRPC(endpoint string) (*rpc.Client, error) {
	if endpoint == "" {
		endpoint = node.DefaultIPCEndpoint(clientIdentifier)
	} else if strings.HasPrefix(endpoint, "rpc:") || strings.HasPrefix(endpoint, "ipc:") {
		// Backwards compatibility with glemo < 1.5 which required
		// these prefixes.
		endpoint = endpoint[4:]
	}
...
// ephemeralConsole starts a new glemo node, attaches an ephemeral JavaScript
