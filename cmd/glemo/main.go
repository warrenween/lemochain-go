...
// glemo is the official command-line client for Lemochain.
package main
...
const (
	clientIdentifier = "glemo" // Client identifier to advertise over the network
)

var (
	// Git SHA1 commit hash of the release (set via linker flags)
	gitCommit = ""
	// Lemochain address of the Glemo release oracle.
	relOracle = common.HexToAddress("0xfa7b9770ca4cb04296cac84f37736d4041251cdf")
	// The app that holds all commands and flags.
	app = utils.NewApp(gitCommit, "the lemochain-go command line interface")
...
// glemo is the main entry point into the system if no special subcommand is ran.
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	go func() {
		...
		stateReader := lemoclient.NewClient(rpcClient)
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		// Mining only makes sense if a full Lemochain node is running
		if ctx.GlobalBool(utils.LightModeFlag.Name) || ctx.GlobalString(utils.SyncModeFlag.Name) == "light" {
			utils.Fatalf("Light clients do not support mining")
		}
		var lemochain *lemo.Lemochain
		if err := stack.Service(&lemochain); err != nil {
			utils.Fatalf("Lemochain service not running: %v", err)
		}
		...
		if threads := ctx.GlobalInt(utils.MinerThreadsFlag.Name); threads > 0 {
			...
			if th, ok := lemochain.Engine().(threaded); ok {
				th.SetThreads(threads)
			}
		}
		// Set the gas price to the limits from the CLI and start mining
		lemochain.TxPool().SetGasPrice(utils.GlobalBig(ctx, utils.GasPriceFlag.Name))
		if err := lemochain.StartMining(true); err != nil {
