...
// Package utils contains internal helper functions for lemochain-go commands.
package utils
...
import (
	...
	"github.com/LemoFoundationLtd/lemochain-go/common/fdlimit"
	...
	"github.com/LemoFoundationLtd/lemochain-go/dashboard"
	...
	whisper "github.com/LemoFoundationLtd/lemochain-go/whisper/whisperv6"
...
var (
	...
	NetworkIdFlag = cli.Uint64Flag{
		...
		Value: lemo.DefaultConfig.NetworkId,
	}
	...
	defaultSyncMode = lemo.DefaultConfig.SyncMode
	...
	LightPeersFlag = cli.IntFlag{
		...
		Value: lemo.DefaultConfig.LightPeers,
	}
	...
	TxPoolPriceLimitFlag = cli.Uint64Flag{
		...
		Value: lemo.DefaultConfig.TxPool.PriceLimit,
	}
	TxPoolPriceBumpFlag = cli.Uint64Flag{
		Name:  "txpool.pricebump",
		Usage: "Price bump percentage to replace an already existing transaction",
		Value: lemo.DefaultConfig.TxPool.PriceBump,
	}
	TxPoolAccountSlotsFlag = cli.Uint64Flag{
		Name:  "txpool.accountslots",
		Usage: "Minimum number of executable transaction slots guaranteed per account",
		Value: lemo.DefaultConfig.TxPool.AccountSlots,
	}
	TxPoolGlobalSlotsFlag = cli.Uint64Flag{
		Name:  "txpool.globalslots",
		Usage: "Maximum number of executable transaction slots for all accounts",
		Value: lemo.DefaultConfig.TxPool.GlobalSlots,
	}
	TxPoolAccountQueueFlag = cli.Uint64Flag{
		Name:  "txpool.accountqueue",
		Usage: "Maximum number of non-executable transaction slots permitted per account",
		Value: lemo.DefaultConfig.TxPool.AccountQueue,
	}
	TxPoolGlobalQueueFlag = cli.Uint64Flag{
		Name:  "txpool.globalqueue",
		Usage: "Maximum number of non-executable transaction slots for all accounts",
		Value: lemo.DefaultConfig.TxPool.GlobalQueue,
	}
	TxPoolLifetimeFlag = cli.DurationFlag{
		Name:  "txpool.lifetime",
		Usage: "Maximum amount of time non-executable transaction are queued",
		Value: lemo.DefaultConfig.TxPool.Lifetime,
	}
	...
	GasPriceFlag = BigFlag{
		...
		Value: lemo.DefaultConfig.GasPrice,
	}
	...
	GpoBlocksFlag = cli.IntFlag{
		...
		Value: lemo.DefaultConfig.GPO.Blocks,
	}
	GpoPercentileFlag = cli.IntFlag{
		Name:  "gpopercentile",
		Usage: "Suggested gas price is the given percentile of a set of recent transaction gas prices",
		Value: lemo.DefaultConfig.GPO.Percentile,
	}
...
// for Glemo and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `glemo account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	lemoPeers := cfg.MaxPeers - lightPeers
	if lightClient {
		lemoPeers = 0
	}
	log.Info("Maximum peer count", "ETH", lemoPeers, "LES", lightPeers, "total", cfg.MaxPeers)
...
func RegisterLemoService(stack *node.Node, cfg *lemo.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the Lemochain service: %v", err)
	}
}
...
func RegisterLemoStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		ctx.Service(&lemoServ)

		var lesServ *les.LightLemochain
		ctx.Service(&lesServ)

		return lemostats.New(url, lemoServ, lesServ)
	}); err != nil {
		Fatalf("Failed to register the Lemochain Stats service: %v", err)
	}
}
...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node) lemodb.Database {
	var (
...
func MakeChain(ctx *cli.Context, stack *node.Node) (chain *core.BlockChain, chainDb lemodb.Database) {
	var err error
	...
	cache := &core.CacheConfig{
		...
		TrieNodeLimit: lemo.DefaultConfig.TrieCache,
		TrieTimeLimit: lemo.DefaultConfig.TrieTimeout,
	}
	if ctx.GlobalIsSet(CacheFlag.Name) || ctx.GlobalIsSet(CacheGCFlag.Name) {
