...
var tomlSettings = toml.Config{
	...
	MissingField: func(rt reflect.Type, field string) error {
		...
		if unicode.IsUpper(rune(rt.Name()[0])) && rt.PkgPath() != "main" {
			link = fmt.Sprintf(", check github.com/LemoFoundationLtd/lemochain-go/swarm/api/config.go for available fields")
		}
...
func initSwarmNode(config *bzzapi.Config, stack *node.Node, ctx *cli.Context) {
	...
	//set the resolved config path (glemo --datadir)
...
func configFileOverride(config *bzzapi.Config, ctx *cli.Context) (*bzzapi.Config, error) {
	...
	//only do somlemoing if the -config flag has been set
...
func checkDeprecated(ctx *cli.Context) {
	// exit if the deprecated --lemoapi flag is set
	if ctx.GlobalString(DeprecatedLemoAPIFlag.Name) != "" {
		utils.Fatalf("--lemoapi is no longer a valid command line flag, please use --ens-api and/or --swap-api.")
	}
...
func printConfig(config *bzzapi.Config) string {
	...
	if err != nil {
		return fmt.Sprintf("Somlemoing is not right with the configuration: %v", err)
	}
	return string(out)
}
