...
// faucet is a Lemoer faucet backed by a light client.
package main
...
import (
	...
	"github.com/LemoFoundationLtd/lemochain-go/lemostats"
	...
	"github.com/LemoFoundationLtd/lemochain-go/p2p/nat"
...
var (
	...
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Lemoers to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Lemochain and the faucet")
)

var (
	lemoer = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Lemoers", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *lemoPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Lemochain address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Lemochain light client.
type faucet struct {
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Lemochain light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := lemo.DefaultConfig
	...
	// Assemble the lemostats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightLemochain
			ctx.Service(&serv)
			return lemostats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := lemoclient.NewClient(api)
...
// close terminates the Lemochain connection and tears down the faucet.
...
// apiHandler handles requests for Lemoer grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, lemoer),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the Lemochain address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Somlemoing funky happened, please open an issue at https://github.com/LemoFoundationLtd/lemochain-go/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), lemoer)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance = new(big.Int).Div(balance, lemoer)
...
// the username, avatar URL and Lemochain address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for Lemochain addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Lemochain address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Lemochain address and profile URL.
...
// returning the username, avatar URL and Lemochain address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Lemochain address,
