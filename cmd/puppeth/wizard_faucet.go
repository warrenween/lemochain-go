...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Lemoers to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.lemostats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.lemostats = w.readString() + ":" + w.conf.lemostats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.lemostats)
		infos.node.lemostats = w.readDefaultString(infos.node.lemostats) + ":" + w.conf.lemostats
	}
