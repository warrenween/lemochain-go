...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.Genesis.Config.Ethash != nil && !boot {
		...
		if infos.lemohashdir == "" {
			...
			infos.lemohashdir = w.readString()
		} else {
			fmt.Printf("Where should the lemohash mining DAGs be stored on the remote machine? (default = %s)\n", infos.lemohashdir)
			infos.lemohashdir = w.readDefaultString(infos.lemohashdir)
		}
	}
	...
	if infos.lemostats == "" {
		...
		infos.lemostats = w.readString() + ":" + w.conf.lemostats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.lemostats)
		infos.lemostats = w.readDefaultString(infos.lemostats) + ":" + w.conf.lemostats
	}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Lemochain node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
