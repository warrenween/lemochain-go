...
// config contains all the configurations needed by pupplemo that should be saved
// between sessions.
type config struct {
	path      string   // File containing the configuration values
	bootnodes []string // Bootnodes to always connect to by all nodes
	lemostats  string   // Ethstats settings to cache for node deploys
...
func (c config) flush() {
	...
	if err := ioutil.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save pupplemo configs", "file", c.path, "err", err)
	}
}
...
type wizard struct {
	...
	services map[string][]string   // Lemochain services known to be running on servers
...
// converts it to an Lemochain address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by lemostats, which compares IPs textually, not by value.
