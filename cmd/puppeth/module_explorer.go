...
FROM pupplemo/explorer:latest

ADD lemostats.json /lemostats.json
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(explorerLemostats)).Execute(lemostats, map[string]interface{}{
		...
		"Host":   config.lemostats[strings.Index(config.lemostats, "@")+1:],
	})
	files[filepath.Join(workdir, "lemostats.json")] = lemostats.Bytes()
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.lemochain"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
