...
// lemostatsDockerfile is the Dockerfile required to build an lemostats backend
// and associated monitoring site.
var lemostatsDockerfile = `
FROM pupplemo/lemostats:latest

RUN echo 'module.exports = {trusted: [{{.Trusted}}], banned: [{{.Banned}}], reserved: ["yournode"]};' > lib/utils/config.js
`

// lemostatsComposefile is the docker-compose.yml file required to deploy and
// maintain an lemostats monitoring site.
var lemostatsComposefile = `
version: '2'
services:
  lemostats:
    build: .
    image: {{.Network}}/lemostats{{if not .VHost}}
    ports:
      - "{{.Port}}:3000"{{end}}
    environment:
...
func deployEthstats(client *sshClient, network string, port int, secret string, vhost string, trusted []string, banned []string, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(lemostatsDockerfile)).Execute(dockerfile, map[string]interface{}{
	...
	template.Must(template.New("").Parse(lemostatsComposefile)).Execute(composefile, map[string]interface{}{
		"Network": network,
		"Port":    port,
		"Secret":  secret,
		"VHost":   vhost,
	...
	// Build and deploy the lemostats service
	if nocache {
...
// lemostatsInfos is returned from an lemostats status check to allow reporting
// various configuration parameters.
type lemostatsInfos struct {
	host   string
	port   int
	secret string
	config string
...
func (info *lemostatsInfos) Report() map[string]string {
