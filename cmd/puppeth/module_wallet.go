...
FROM pupplemo/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'glemo --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'glemo --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --lemostats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh
...
// checkWallet does a health-check against web wallet server to verify whlemoer
// it's running, and if yes, whlemoer it's responsive.
