...
// nodeDockerfile is the Dockerfile required to run an Lemochain node.
var nodeDockerfile = `
FROM lemochain/client-go:latest
...
// an Lemochain node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
...
// deployNode deploys a new Lemochain node container to a remote machine via SSH,
...
// whlemoer it's running, and if yes, whlemoer it's responsive.
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 glemo --exec admin.nodeInfo.id attach", network, kind)); err != nil {
