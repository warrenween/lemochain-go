...
func (w *wizard) deployLemostats() {
	...
	fmt.Println()
	fmt.Printf("Which port should lemostats listen on? (default = %d)\n", infos.port)
	infos.port = w.readDefaultInt(infos.port)

	// Figure which virtual-host to deploy lemostats on
	if infos.host, err = w.ensureVirtualHost(client, infos.port, infos.host); err != nil {
		log.Error("Failed to decide on lemostats host", "err", err)
		return
	}
	// Port and proxy settings retrieved, figure out the secret and boot lemostats
	fmt.Println()
	if infos.secret == "" {
	...
	// Try to deploy the lemostats server on the host
	nocache := false
	if existed {
		fmt.Println()
		fmt.Printf("Should the lemostats be built from scratch (y/n)? (default = no)\n")
