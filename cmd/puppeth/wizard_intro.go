...
// makeWizard creates and returns a new pupplemo wizard.
...
// setting up a new or managing an existing Lemochain private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to pupplemo, your Lemochain private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new Lemochain network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and lemostats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Pupplemo uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering Lemochain network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".pupplemo", w.network)
	...
	for {
		...
		default:
			log.Error("That's not somlemoing I can do")
		}
	}
}
