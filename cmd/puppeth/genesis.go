...
// cppLemochainGenesisSpec represents the genesis specification format used by the
// C++ Lemochain implementation.
type cppLemochainGenesisSpec struct {
	...
	Accounts map[common.Address]*cppLemochainGenesisSpecAccount `json:"accounts"`
}
...
type cppLemochainGenesisSpecLinearPricing struct {
	Base uint64 `json:"base"`
	Word uint64 `json:"word"`
}

// newCppLemochainGenesisSpec converts a lemochain-go genesis block into a Parity specific
...
func newCppLemochainGenesisSpec(network string, genesis *core.Genesis) (*cppLemochainGenesisSpec, error) {
	...
	spec.Params.BlockReward = (*hexutil.Big)(lemohash.FrontierBlockReward)
	...
	spec.Accounts = make(map[common.Address]*cppLemochainGenesisSpecAccount)
	for address, account := range genesis.Alloc {
		spec.Accounts[address] = &cppLemochainGenesisSpecAccount{
			Balance: (*hexutil.Big)(account.Balance),
			Nonce:   account.Nonce,
		}
	}
	spec.Accounts[common.BytesToAddress([]byte{1})].Precompiled = &cppLemochainGenesisSpecBuiltin{
		Name: "ecrecover", Linear: &cppLemochainGenesisSpecLinearPricing{Base: 3000},
	}
	spec.Accounts[common.BytesToAddress([]byte{2})].Precompiled = &cppLemochainGenesisSpecBuiltin{
		Name: "sha256", Linear: &cppLemochainGenesisSpecLinearPricing{Base: 60, Word: 12},
	}
	spec.Accounts[common.BytesToAddress([]byte{3})].Precompiled = &cppLemochainGenesisSpecBuiltin{
		Name: "ripemd160", Linear: &cppLemochainGenesisSpecLinearPricing{Base: 600, Word: 120},
	}
	spec.Accounts[common.BytesToAddress([]byte{4})].Precompiled = &cppLemochainGenesisSpecBuiltin{
		Name: "identity", Linear: &cppLemochainGenesisSpecLinearPricing{Base: 15, Word: 3},
	}
	if genesis.Config.ByzantiumBlock != nil {
		spec.Accounts[common.BytesToAddress([]byte{5})].Precompiled = &cppLemochainGenesisSpecBuiltin{
			Name: "modexp", StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64()),
		}
		spec.Accounts[common.BytesToAddress([]byte{6})].Precompiled = &cppLemochainGenesisSpecBuiltin{
			Name: "alt_bn128_G1_add", StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64()), Linear: &cppLemochainGenesisSpecLinearPricing{Base: 500},
		}
		spec.Accounts[common.BytesToAddress([]byte{7})].Precompiled = &cppLemochainGenesisSpecBuiltin{
			Name: "alt_bn128_G1_mul", StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64()), Linear: &cppLemochainGenesisSpecLinearPricing{Base: 40000},
		}
		spec.Accounts[common.BytesToAddress([]byte{8})].Precompiled = &cppLemochainGenesisSpecBuiltin{
...
// newParityChainSpec converts a lemochain-go genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	...
	spec.Genesis.Seal.Lemochain.Nonce = (hexutil.Bytes)(make([]byte, 8))
	binary.LittleEndian.PutUint64(spec.Genesis.Seal.Lemochain.Nonce[:], genesis.Nonce)

	spec.Genesis.Seal.Lemochain.MixHash = (hexutil.Bytes)(genesis.Mixhash[:])
	spec.Genesis.Difficulty = (*hexutil.Big)(genesis.Difficulty)
	spec.Genesis.Author = genesis.Coinbase
	spec.Genesis.Timestamp = (hexutil.Uint64)(genesis.Timestamp)
	spec.Genesis.ParentHash = genesis.ParentHash
...
// pyLemochainGenesisSpec represents the genesis specification format used by the
// Python Lemochain implementation.
type pyLemochainGenesisSpec struct {
	Nonce      hexutil.Bytes     `json:"nonce"`
	Timestamp  hexutil.Uint64    `json:"timestamp"`
	ExtraData  hexutil.Bytes     `json:"extraData"`
	GasLimit   hexutil.Uint64    `json:"gasLimit"`
...
// newPyLemochainGenesisSpec converts a lemochain-go genesis block into a Parity specific
// chain specification format.
