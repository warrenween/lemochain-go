...
ENTRYPOINT [ \
	"faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--lemostats", "{{.Lemostats}}", "--lemoport", "{{.LemoPort}}",     \
...
services:
  faucet:
    ...
    ports:
      - "{{.LemoPort}}:{{.LemoPort}}"{{if not .VHost}}
      - "{{.ApiPort}}:8080"{{end}}
    volumes:
      - {{.Datadir}}:/root/.faucet
    environment:
...
func (info *faucetInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Funding amount (base tier)":   fmt.Sprintf("%d Lemoers", info.amount),
