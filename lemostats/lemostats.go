...
// Package lemostats implements the network stats reporting service.
package lemostats
...
func New(url string, lemoServ *lemo.Lemochain, lesServ *les.LightLemochain) (*Service, error) {
...
func (s *Service) loop() {
	...
	var txpool txPool
	if s.lemo != nil {
		blockchain = s.lemo.BlockChain()
		txpool = s.lemo.TxPool()
	} else {
	...
	for {
		...
		if len(msg["emit"]) == 2 && command == "history" {
			...
			if !ok {
				...
				s.histCh <- nil
				continue // Lemostats sometime sends invalid history requests, ignore those
			}
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["lemo"]; info != nil {
		network = fmt.Sprintf("%d", info.(*lemo.NodeInfo).Network)
		protocol = fmt.Sprintf("lemo/%d", lemo.ProtocolVersions[0])
	} else {
...
func (s *Service) reportLatency(conn *websocket.Conn) error {
	// Send the current time to the lemostats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to lemostats", "latency", latency)
...
func (s *Service) reportBlock(conn *websocket.Conn, block *types.Block) error {
	...
	log.Trace("Sending new block to lemostats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) assembleBlockStats(block *types.Block) *blockStats {
	...
	if s.lemo != nil {
		...
		if block == nil {
			block = s.lemo.BlockChain().CurrentBlock()
		}
		header = block.Header()
		td = s.lemo.BlockChain().GetTd(header.Hash(), header.Number.Uint64())
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(list) > 0 {
		...
		var head int64
		if s.lemo != nil {
			head = s.lemo.BlockChain().CurrentHeader().Number.Int64()
		} else {
	...
	for i, number := range indexes {
		...
		if s.lemo != nil {
			block = s.lemo.BlockChain().GetBlockByNumber(number)
		} else {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to lemostats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *websocket.Conn) error {
	...
	var pending int
	if s.lemo != nil {
		pending, _ = s.lemo.TxPool().Stats()
	} else {
		pending = s.les.TxPool().Stats()
	}
	// Assemble the transaction stats and send it to the server
	log.Trace("Sending pending transactions to lemostats", "count", pending)
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	if s.lemo != nil {
		mining = s.lemo.Miner().Mining()
		hashrate = int(s.lemo.Miner().HashRate())

		sync := s.lemo.Downloader().Progress()
		syncing = s.lemo.BlockChain().CurrentHeader().Number.Uint64() >= sync.HighestBlock

		price, _ := s.lemo.ApiBackend.SuggestPrice(context.Background())
	...
	log.Trace("Sending node details to lemostats")
