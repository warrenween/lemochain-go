...
import (
	...
	"github.com/LemoFoundationLtd/lemochain-go/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of glemo is "glemo". If no
	...
	// registered services, instead those can use utility mlemoods to create/access
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Glemo", keep that.
...
// These resources are resolved differently for "glemo" instances.
var isOldGlemoResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) resolvePath(path string) string {
	...
	// by glemo 1.4 are used if they exist.
	if c.name() == "glemo" && isOldGlemoResource[path] {
		oldpath := ""
		if c.Name == "glemo" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "lemochain-go-keystore")
		ephemeral = keydir
	}
