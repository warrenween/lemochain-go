...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Glemo.LightScryptN, Glemo.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.lemochain", new NodeConfig());
			node.start();
			...
			LemochainClient ec = node.getLemochainClient();
	...
	// signatures too. Regression test for lemochain-go issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altoglemo
	...
	workspace, err := ioutil.TempDir("", "glemo-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/lemochain/glemotest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Glemo and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.lemochain", "github.com/LemoFoundationLtd/lemochain-go/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "glemo.aar"), "glemo.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "lemochain", "glemotest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.lemochain.glemotest"
