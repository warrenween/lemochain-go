...
import (
	...
	"github.com/LemoFoundationLtd/lemochain-go/accounts/usbwallet/internal/trezor"
...
type trezorDriver struct {
	...
	pinwait bool          // Flags whlemoer the device is waiting for PIN entry
...
// Status implements accounts.Wallet, always whlemoer the Trezor is opened, closed
// or whlemoer the Lemochain app was not started on it.
...
// Lemochain address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.LemochainAddress)
	if _, err := w.trezorExchange(&trezor.LemochainGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.LemochainSignTx{
	...
	response := new(trezor.LemochainTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.LemochainTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
...
// mlemood will also return the index of the destination object used.
