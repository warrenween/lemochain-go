...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Lemochain
...
type wallet struct {
	...
	deriveChain    lemochain.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Lemochain account
			...
			// If the next account is empty, stop self-derivation, but add it nonlemoeless
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain lemochain.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
