## Go Lemochain Dashboard

The dashboard is a data visualizer integrated into glemo, intended to collect and visualize useful information of an Lemochain node. It consists of two parts:
...
As the dashboard depends on certain NPM packages (which are not included in the `lemochain-go` repo), these need to be installed first:

```
$ (cd dashboard/assets && yarn install && yarn flow)
```

Normally the dashboard assets are bundled into Glemo via `go-bindata` to avoid external dependencies. Rebuilding Glemo after each UI modification however is not feasible from a developer perspective. Instead, we can run `yarn dev` to watch for file system changes and refresh the browser automatically.

```
$ glemo --dashboard --vmodule=dashboard=5
$ (cd dashboard/assets && yarn dev)
```

To bundle up the final UI into Glemo, run `go generate`:
