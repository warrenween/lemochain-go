...
import (
	...
	"github.com/LemoFoundationLtd/lemochain-go/internal/web3ext"
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jlemo", struct{}{})

	jlemoObj, _ := c.jsre.Get("jlemo")
	jlemoObj.Object().Set("send", bridge.Send)
	jlemoObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jlemo);"); err != nil {
	...
	flatten := "var lemo = web3.lemo; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	if c.prompter != nil {
		...
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jlemo.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jlemo.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jlemo.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jlemo.sign = personal.sign;`); err != nil {
...
func (c *Console) Welcome() {
	...
	fmt.Fprintf(c.printer, "Welcome to the Glemo JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + lemo.coinbase);
		console.log("at block: " + lemo.blockNumber + " (" + new Date(1000 * lemo.getBlock(lemo.blockNumber).timestamp) + ")");
