...
// Package core implements the Lemochain consensus protocol.
package core
...
type CacheConfig struct {
	Disabled      bool          // Whlemoer to disable trie write caching (archive node)
...
// available in the database. It initialises the default Lemochain Validator and
// Processor.
func NewBlockChain(db lemodb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	if cacheConfig == nil {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= lemodb.IdealBatchSize {
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	for i, block := range chain {
		...
		case err == ErrKnownBlock:
			...
			// this number we did a rollback and we should reimport it nonlemoeless.
