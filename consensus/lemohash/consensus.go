...
func (lemohash *Lemohash) VerifyHeader(chain consensus.ChainReader, header *types.Header, seal bool) error {
	...
	return lemohash.verifyHeader(chain, header, parent, false, seal)
}
	...
	for i := 0; i < workers; i++ {
		go func() {
			for index := range inputs {
				errors[index] = lemohash.verifyHeaderWorker(chain, headers, seals, index)
				done <- index
			}
		}()
	}
...
func (lemohash *Lemohash) verifyHeaderWorker(chain consensus.ChainReader, headers []*types.Header, seals []bool, index int) error {
	...
	return lemohash.verifyHeader(chain, headers[index], parent, false, seals[index])
}
...
func (lemohash *Lemohash) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
	...
	for _, uncle := range block.Uncles() {
		...
		if err := lemohash.verifyHeader(chain, uncle, ancestors[uncle.ParentHash], true, true); err != nil {
			return err
		}
	}
	return nil
...
func (lemohash *Lemohash) verifyHeader(chain consensus.ChainReader, header, parent *types.Header, uncle bool, seal bool) error {
	...
	expected := lemohash.CalcDifficulty(chain, header.Time.Uint64(), parent)
	...
	if seal {
		if err := lemohash.VerifySeal(chain, header); err != nil {
			return err
		}
	}
...
func calcDifficultyByzantium(time uint64, parent *types.Header) *big.Int {
	// https://github.com/lemochain/EIPs/issues/100.
	// algorithm:
	// diff = (parent_diff +
	...
	//   https://github.com/lemochain/EIPs/pull/669
...
func calcDifficultyHomestead(time uint64, parent *types.Header) *big.Int {
	// https://github.com/lemochain/EIPs/blob/master/EIPS/eip-2.md
	// algorithm:
	// diff = (parent_diff +
...
func (lemohash *Lemohash) VerifySeal(chain consensus.ChainReader, header *types.Header) error {
	...
	if lemohash.config.PowMode == ModeFake || lemohash.config.PowMode == ModeFullFake {
		...
		if lemohash.fakeFail == header.Number.Uint64() {
	...
	if lemohash.shared != nil {
		return lemohash.shared.VerifySeal(chain, header)
	}
	...
	cache := lemohash.cache(number)
	size := datasetSize(number)
	if lemohash.config.PowMode == ModeTest {
		size = 32 * 1024
	}
...
func (lemohash *Lemohash) Prepare(chain consensus.ChainReader, header *types.Header) error {
	...
	header.Difficulty = lemohash.CalcDifficulty(chain, header.Time.Uint64(), parent)
	return nil
}
