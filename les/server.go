...
import (
	...
	"github.com/LemoFoundationLtd/lemochain-go/les/flowcontrol"
...
func NewLesServer(lemo *lemo.Lemochain, config *lemo.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(lemo.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, lemo.EventMux(), lemo.Engine(), newPeerSet(), lemo.BlockChain(), lemo.TxPool(), lemo.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(lemo.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(lemo.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(lemo.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(lemo.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(lemo.ChainDb())
	return srv, nil
}
...
func newCostStats(db lemodb.Database) *requestCostStats {
