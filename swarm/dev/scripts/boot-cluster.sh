...
#
# The cluster contains a bootnode, a glemo node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the glemo and
# swarm nodes
start_bootnode() {
...
# start_glemo_node starts a glemo node with --datadir pointing at <base-dir>/glemo
# and a single, unlocked account with password "glemo"
start_glemo_node() {
  local dir="${base_dir}/glemo"
  mkdir -p "${dir}"

  local password="glemo"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating glemo account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get glemo account address"
  fi
  ...
  start_node "glemo" "${GETH_IP}" "$(which glemo)" ${args[@]}
}
...
create_node_network() {
  ...
  # create the vlemo pair
  local vlemo0="vlemo${name}0"
  local vlemo1="vlemo${name}1"
  ip link add name "${vlemo0}" type vlemo peer name "${vlemo1}"

  # add one end to the bridge
  ip link set dev "${vlemo0}" master "${BRIDGE_NAME}"
  ip link set dev "${vlemo0}" up

  # add the other end to the namespace, rename it lemo0 and give it the ip
  ip link set dev "${vlemo1}" netns "${name}"
  ip netns exec "${name}" ip link set dev "${vlemo1}" name "lemo0"
  ip netns exec "${name}" ip link set dev "lemo0" up
  ip netns exec "${name}" ip address add "${ip}/24" dev "lemo0"
}
...
create_account() {
  ...
  glemo --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
