...
import (
	...
	"github.com/LemoFoundationLtd/lemochain-go/swarm/storage"
)
...
func TestAPIResolve(t *testing.T) {
	...
	tests := []*test{
		...
		},
		{
			...
			dns:       nil,
			addr:      ensAddr,
			expectErr: errors.New(`no DNS to resolve name: "swarm.lemo"`),
		},
		{
		...
		},
		{
			...
			addr:      ensAddr,
			immutable: true,
			expectErr: errors.New(`immutable address not a content hash: "swarm.lemo"`),
		},
		{
			desc:      "DNS configured, ENS address, name doesn't resolve, returns error",
			dns:       doesntResolve,
			addr:      ensAddr,
			expectErr: errors.New(`DNS name not found: "swarm.lemo"`),
		},
	}
	for _, x := range tests {
...
func TestMultiResolver(t *testing.T) {
	...
	lemoAddr := "swarm.lemo"
	lemoHash := "0x2222222222222222222222222222222222222222222222222222222222222222"
	lemoResolve := newTestResolver(lemoHash)
	...
	tests := []struct {
		...
		},
		{
			...
			r:      NewMultiResolver(MultiResolverOptionWithResolver(lemoResolve, "")),
			addr:   lemoAddr,
			result: lemoHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(lemoResolve, ""),
				MultiResolverOptionWithResolver(lemoResolve, ""),
			),
			addr:   lemoAddr,
			result: lemoHash,
		...
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, ""),
				MultiResolverOptionWithResolver(lemoResolve, "lemo"),
			),
			addr:   lemoAddr,
			result: lemoHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, "lemo"),
				MultiResolverOptionWithResolver(doesntResolve, "lemo"),
				MultiResolverOptionWithResolver(lemoResolve, "lemo"),
			),
			addr:   lemoAddr,
		...
		},
		{
			...
			addr:   lemoAddr,
			result: lemoHash,
			err:    errors.New(`DNS name not found: "swarm.lemo"`),
		},
		{
