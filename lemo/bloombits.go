...
const (
	// bloomServiceThreads is the number of goroutines used globally by an Lemochain
...
func (lemo *Lemochain) startBloomHandlers() {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-lemo.shutdownChan:
					return

				case request := <-lemo.bloomRequests:
					task := <-request
					task.Bitsets = make([][]byte, len(task.Sections))
					for i, section := range task.Sections {
						head := core.GetCanonicalHash(lemo.chainDb, (section+1)*params.BloomBitsBlocks-1)
						if compVector, err := core.GetBloomBits(lemo.chainDb, task.Bit, section, head); err == nil {
...
// for the Lemochain header bloom filters, permitting blazing fast filtering.
type BloomIndexer struct {
	size uint64 // section size to generate bloombits for

	db  lemodb.Database       // database instance to write index data and metadata into
...
func NewBloomIndexer(db lemodb.Database, size uint64) *core.ChainIndexer {
	backend := &BloomIndexer{
		db:   db,
		size: size,
	}
	table := lemodb.NewTable(db, string(core.BloomBitsIndexPrefix))
