...
func benchmarkBloomBits(b *testing.B, sectionSize uint64) {
	benchDataDir := node.DefaultDataDir() + "/glemo/chaindata"
	fmt.Println("Running bloombits benchmark   section size:", sectionSize)

	db, err := lemodb.NewLDBDatabase(benchDataDir, 128, 1024)
	if err != nil {
	...
	for i := 0; i < benchFilterCnt; i++ {
		if i%20 == 0 {
			db.Close()
			db, _ = lemodb.NewLDBDatabase(benchDataDir, 128, 1024)
...
func forEachKey(db lemodb.Database, startPrefix, endPrefix []byte, fn func(key []byte)) {
	it := db.(*lemodb.LDBDatabase).NewIterator()
	it.Seek(startPrefix)
	for it.Valid() {
		key := it.Key()
		cmpLen := len(key)
...
func clearBloomBits(db lemodb.Database) {
