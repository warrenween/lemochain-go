...
// Package filters implements an lemochain filtering system for block,
...
type subscription struct {
	...
	typ       Type
	created   time.Time
	logsCrit  lemochain.FilterQuery
	logs      chan []*types.Log
...
func (sub *Subscription) Unsubscribe() {
	...
	uninstallLoop:
		for {
			...
			// the eventLoop broadcast mlemood to deadlock when writing to the
			// filter event channel while the subscription loop is waiting for
			// this mlemood to return (and thus not reading these events).
			select {
		...
		// will probably be closed by the client asap after this mlemood returns.
		<-sub.Err()
	})
}
...
func (es *EventSystem) SubscribeLogs(crit lemochain.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit lemochain.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit lemochain.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit lemochain.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
