...
func testGetNodeData(t *testing.T, protocol int) {
	...
	statedb, _ := lemodb.NewMemDatabase()
...
// Tests that post lemo protocol handshake, DAO fork-enabled clients also execute
...
func testDAOChallenge(t *testing.T, localForked, remoteForked bool, timeout bool) {
	...
	peer, _ := newTestPeer("peer", lemo63, pm, true)
	defer peer.close()
	...
	if !timeout {
		blocks, _ := core.GenerateChain(&params.ChainConfig{}, genesis, lemohash.NewFaker(), db, 1, func(i int, block *core.BlockGen) {
			if remoteForked {
