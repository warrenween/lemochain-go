...
// stateReq represents a batch of state fetch requests groupped toglemoer into
...
type stateReq struct {
	...
	dropped  bool                       // Flag whlemoer the peer dropped off early
}
...
func (d *Downloader) runStateSync(s *stateSync) *stateSync {
	...
	for {
		...
		case req := <-timeout:
			// If the peer is already requesting somlemoing else, ignore the stale timeout.
...
// newStateSync creates a new state trie download scheduler. This mlemood does not
...
func (s *stateSync) loop() error {
	...
	for s.sched.Pending() > 0 {
		...
		s.assignTasks()
		// Tasks assigned, wait for somlemoing to happen
		select {
		case <-newPeer:
...
func (s *stateSync) commit(force bool) error {
	if !force && s.bytesUncommitted < lemodb.IdealBatchSize {
		return nil
	}
	start := time.Now()
...
func (s *stateSync) process(req *stateReq) error {
	...
	for hash, task := range req.tasks {
		// If the node did deliver somlemoing, missing items may be due to a protocol
...
// peer into the state trie, returning whlemoer anything useful was written or any
// error occurred.
