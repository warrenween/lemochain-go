...
type fetchRequest struct {
	...
	From    uint64          // [lemo/62] Requested chain element index (used for skeleton fills only)
	Headers []*types.Header // [lemo/62] Requested headers, sorted by request order
...
type queue struct {
	...
	headerPendPool  map[string]*fetchRequest       // [lemo/62] Currently pending header retrieval operations
	...
	blockTaskQueue *prque.Prque                  // [lemo/62] Priority queue of the headers to fetch the blocks (bodies) for
	blockPendPool  map[string]*fetchRequest      // [lemo/62] Currently pending block (body) retrieval operations
	blockDonePool  map[common.Hash]struct{}      // [lemo/62] Set of the completed block (body) fetches

	receiptTaskPool  map[common.Hash]*types.Header // [lemo/63] Pending receipt retrieval tasks, mapping hashes to headers
	receiptTaskQueue *prque.Prque                  // [lemo/63] Priority queue of the headers to fetch the receipts for
	receiptPendPool  map[string]*fetchRequest      // [lemo/63] Currently pending receipt retrieval operations
	receiptDonePool  map[common.Hash]struct{}      // [lemo/63] Set of the completed receipt fetches
...
// InFlightHeaders retrieves whlemoer there are header fetch requests currently
// in flight.
...
// InFlightBlocks retrieves whlemoer there are block fetch requests currently in
// flight.
...
// InFlightReceipts retrieves whlemoer there are receipt fetch requests currently
// in flight.
...
func (q *queue) ReserveHeaders(p *peerConnection, count int) *fetchRequest {
	...
	// Short circuit if the peer's already downloading somlemoing (sanity check to
	// not corrupt state)
...
// returns a flag whlemoer empty blocks were queued requiring processing.
...
// also returns a flag whlemoer empty receipts were queued requiring importing.
...
// skipping any previously failed ones. This mlemood is a generic version used
// by the individual special reservation functions.
//
// Note, this mlemood expects the queue lock to be already held for writing. The
// reason the lock is not obtained in here is because the parameters already need
// to access the queue, so they already need a lock anyway.
func (q *queue) reserveHeaders(p *peerConnection, count int, taskPool map[common.Hash]*types.Header, taskQueue *prque.Prque,
	pendPool map[string]*fetchRequest, donePool map[common.Hash]struct{}, isNoop func(*types.Header) bool) (*fetchRequest, bool, error) {
	// Short circuit if the pool has been depleted, or if the peer's already
	// downloading somlemoing (sanity check not to corrupt state)
	if taskQueue.Empty() {
...
// Revoke cancels all pending requests belonging to a given peer. This mlemood is
...
//
// Note, this mlemood expects the queue lock to be already held. The
// reason the lock is not obtained in here is because the parameters already need
// to access the queue, so they already need a lock anyway.
...
// cache. This mlemood either accepts all headers it received, or none of them
// if they do not map correctly to the skeleton.
//
// If the headers are accepted, the mlemood makes an attempt to deliver the set
...
// The mlemood returns the number of blocks bodies accepted from the delivery and
...
// The mlemood returns the number of transaction receipts accepted from the delivery
