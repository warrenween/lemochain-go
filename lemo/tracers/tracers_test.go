...
var makeTest = function(tx, rewind) {
  ...
  var block   = lemo.getBlock(lemo.getTransaction(tx).blockHash);
  var genesis = lemo.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.lemo.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  lemo.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
